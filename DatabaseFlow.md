...
|---|---|---|---|---|
| 1 | Purchase arrives (`/api/webhooks/purchase-created` or dev) | `+kyc_masters` `+purchase_events` `+kyc_document_submissions`×N (snapshot) `+reminder_states` `+kyc_links` `+email_logs` | `purchase_webhook_received`, `entity_detected_from_pan`, `kyc_link_generated` | **Same purchaseId again** → no new rows, `~purchase_events.retryCount`, replay snapshot (`retry_same_payload`). **Same purchaseId, different PAN** → `~purchase_events.conflictCount`, `409` (`purchase_id_conflict`). **Same PAN + same buyerName + same mobile, KYC progressed past `link_sent`** (Case 1) → no new KYC, `+purchase_events` (`kyc_bypassed_duplicate_buyer`) + audit row on the existing KYC. **Same PAN + same buyerName + different mobile** (Case 3) → `+kyc_masters` (terminal `cancelled`, `mobileHash` populated), `+purchase_events` (`kyc_logged_duplicate_buyer_different_mobile`); no link / no email — the new mobile is searchable via `?mobile=` on the reviewer / admin dashboards. **Bad/invalid PAN** → nothing written, `400`. **Concurrent dup** → P2002 caught, replays idempotently. All wrapped in one transaction. |
...
| `kyc_masters` | reviewer/admin lists: `order updatedAt desc`; PAN lookup; mobile lookup; `where panHash/order updatedAt desc` (duplicate-buyer check); `where mobileHash` (fraud search) | `[purchaseId]`,`[entityType]`,`[overallStatus]`,`[updatedAt]`,`[mobileHash]` | ✅ good — mobileHash added for the duplicate-buyer fraud-search trail |
